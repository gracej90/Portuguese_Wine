...
```{r echo=FALSE, warning=FALSE, message=FALSE, Univariate_Plots_0}
...
#Distributions of key variables by wine.color: Alcohol, pH, volatile acidity, residual sugar
...
#transform residual.sugar using scale_x_log10 as the graph is right skewed
...
ggplot(aes(x= quality.fac, y= alcohol), data = wine) +
...
ggplot(aes(x= quality.fac, y= volatile.acidity), data = wine) +
  geom_boxplot(aes(color = wine.color)) +
  coord_cartesian(ylim = c(0, 1.6))

#Box plot: Residual.sugar & Quality
ggplot(aes(x= quality.fac, y= residual.sugar), data = wine) +
  geom_boxplot(aes(color = wine.color)) +
...
set.seed(90)
names(wine)
wine_subset <- wine[, c(2,3,4,5,8,9,11,13, 14)]
ggpairs(wine_subset[sample.int(nrow(wine_subset), 1000),], axisLabels = 'internal')
...
As citric.acid level increases, volatile.acidity level decreases. The mean summary line (shown in red), clearly drops between citric.acid level of 0.1~0.2. Also, graph become noisier as citric.acid level approach 1.
...
ggplot(aes(x= residual.sugar, y = quality.fac), data = wine) +
...
### Description Two
As citric acid level increases, volatile acidity level decreases for both Red and White wine. The graph becomes noisier as citric acid approaches 1 g/dm^3, but the mean summary line shows a negative correlation overall. This is interesting as citric acid known to add 'freshness' is inversely correlated to volatile acididty known to cause unpleasant, vinegar taste.
...
ggplot(aes(x= residual.sugar, y = quality.fac), data = wine) +
  ...
  ylab("Quality (rating between 0 and 10)") + 
  guides(colour = guide_legend("Alcohol Strength")) +
  ggtitle("Quality by Residual Sugar (log10) and Alcohol Strength")
...
There was a clear trend between alcohol strength, residual sugar, and quality. Wine samples with higher amounts of residual sugar, which correlates to lower alcohol level had lower quality ratings. It was suprising to find that pH has close to 0 correlation with quality, but it became clear that most wines are between 3-4 on the pH scale. A difficulty I had was finding the best way to transform the plots. By using a combination of scale_x_log10 and coord_trans, I was able to show the relationships between variables more clearly.
